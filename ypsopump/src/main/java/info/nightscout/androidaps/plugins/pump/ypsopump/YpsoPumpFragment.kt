...
class YpsoPumpFragment : DaggerFragment() {
    ...
    init {
        refreshLoop = Runnable {
            activity?.runOnUiThread { updateGUI(UpdateGui.Full) }
    ...
    override fun onResume() {
        ...
        disposable += rxBus
            ...
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ updateGUI(UpdateGui.TreatmentValues) }, { fabricPrivacy.logException(it) })
        disposable += rxBus
            .toObservable(EventTempBasalChange::class.java)
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ updateGUI(UpdateGui.TreatmentValues) }, { fabricPrivacy.logException(it) })
        disposable += rxBus
            .toObservable(EventPumpConfigurationDisplayChanged::class.java)
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({
                updateGUI(UpdateGui.Configuration)
        ...
        disposable += rxBus
            ...
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ updateGUI(UpdateGui.Queue) }, { fabricPrivacy.logException(it) })
        disposable += rxBus
    ...
    @Synchronized
    fun updateGUI(updateType: UpdateGui) {
        ...
        if (updateType == UpdateGui.Status || updateType == UpdateGui.Full) {
            // Pump Status (Error)

            //if (pumpUtil.driverStatus
            // TODO error handling

            var pumpStatus: YpsoDriverStatus? = pumpUtil.driverStatus

            when (pumpStatus) {
                null,
                YpsoDriverStatus.Sleeping                   -> pump_status.text = "{fa-bed}   "
                YpsoDriverStatus.Connecting,
                YpsoDriverStatus.Disconnecting              -> pump_status.text = "{fa-bluetooth-b spin}   " + resourceHelper.gs(pumpStatus.resourceId)
                YpsoDriverStatus.Connected,
                YpsoDriverStatus.Disconnected               -> pump_status.text = "{fa-bluetooth-b}   " + resourceHelper.gs(pumpStatus.resourceId)

                YpsoDriverStatus.ErrorCommunicatingWithPump -> {
        ...
        if (updateType == UpdateGui.Queue || updateType == UpdateGui.Full) {
            // Queue
            val status = commandQueue.spannedStatus()
            if (status.toString() == "") {
        ...
        if (updateType == UpdateGui.TreatmentValues || updateType == UpdateGui.Full) {
        ...
        if (updateType == UpdateGui.Configuration || updateType == UpdateGui.Full) {
            // Firmware, Errors
        ...
        if (updateType == UpdateGui.OtherValues || updateType == UpdateGui.Full) {
            // Battery, Reservoir
